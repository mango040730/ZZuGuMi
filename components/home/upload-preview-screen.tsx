...
export function UploadPreviewScreen({ 
  ...
  const handleUploadClick = async () => {
    ...
    if (!capturedImage || (strokes.length === 0 && currentStroke.length === 0)) {
    ...
    <div className="fixed inset-0 bg-black z-50 flex flex-col animate-in fade-in duration-300">
        ...
        <div className="relative w-full max-w-sm">
          ...
          >
            ...
            {(isTextMode || questionText.length > 0) && (
              <div className={`absolute top-0 left-0 right-0 p-6 flex flex-col ${!isTextMode ? 'pointer-events-none' : ''}`}>
                <textarea
                  ref={textareaRef}
                  value={questionText}
                  onChange={(e) => setQuestionText(e.target.value)}
                  placeholder={isTextMode ? "궁금한 점을 적어보세요" : ""}
                  className="w-full bg-transparent text-white text-3xl text-left font-bold leading-normal resize-none outline-none placeholder:text-white/60 drop-shadow-xl"
                  rows={1}
          ...
          <div className="absolute -right-12 top-1/2 -translate-y-1/2 flex flex-col gap-4">
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
                isTextMode ? "bg-white text-black" : "bg-white/20 text-white"
              }`}
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
                isMosaicMode ? "bg-white text-black" : "bg-white/20 text-white"
              }`}
              aria-label="Mosaic mode"
            >
      ...
      <div className="flex justify-center pb-12 z-50">
        <button
          ...
          className="px-8 py-3 bg-[#3a3a3a] rounded-full text-white text-sm font-medium hover:bg-[#4a4a4a] transition-colors disabled:opacity-50"
        >
          업로드
        </button>
      </div>
