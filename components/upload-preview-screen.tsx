...
export function UploadPreviewScreen({ 
  ...
  const [isEraserMode, setIsEraserMode] = useState(false) // 지우개 모드 추가
      ...
      <div className="flex-1 w-full px-4 pb-2 min-h-0 flex justify-center items-center">
        <div 
          ...
          className={`relative h-full max-h-[75vh] aspect-[22/29] bg-zinc-900 rounded-3xl overflow-hidden shrink-0 ${
            step === "mosaic" && (isMosaicMode || isEraserMode) ? "touch-none cursor-crosshair" : ""
          }`}
          onPointerDown={(e) => {
            if (step === "mosaic" && (isMosaicMode || isEraserMode)) {
          ...
          onPointerMove={(e) => {
            if (step === "mosaic" && (isMosaicMode || isEraserMode) && isPointerDown) {
              const rect = e.currentTarget.getBoundingClientRect()
        ...
        >
          ...
          {step === "mosaic" && (
            ...
            >
              ...
              <button 
                onClick={handleEraserIconClick} 
                className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-colors ${
                  isEraserMode ? "bg-white" : "bg-black/40 backdrop-blur-md border border-white/20"
                }`}
              >
                <svg width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M16.7574 11.7574L9.75736 4.75736M19.7574 19.7574H6.75736M9.69462 18.8201L18.3632 10.1515C19.5513 8.96342 20.1453 8.3694 20.3679 7.68441C20.5637 7.08188 20.5637 6.43284 20.3679 5.83031C20.1453 5.14532 19.5513 4.5513 18.3632 3.36325L18.1515 3.15147C16.9634 1.96342 16.3694 1.3694 15.6844 1.14683C15.0819 0.951057 14.4328 0.951057 13.8303 1.14683C13.1453 1.3694 12.5513 1.96342 11.3632 3.15147L3.15147 11.3632C1.96342 12.5513 1.3694 13.1453 1.14683 13.8303C0.951057 14.4328 0.951057 15.0819 1.14683 15.6844C1.3694 16.3694 1.96342 16.9634 3.15147 18.1515L3.8201 18.8201C4.166 19.166 4.33896 19.339 4.54079 19.4626C4.71973 19.5723 4.91482 19.6531 5.1189 19.7021C5.34907 19.7574 5.59366 19.7574 6.08284 19.7574H7.43188C7.92106 19.7574 8.16565 19.7574 8.39582 19.7021C8.5999 19.6531 8.79498 19.5723 8.97393 19.4626C9.17576 19.339 9.34871 19.166 9.69462 18.8201Z" 
                    stroke={isEraserMode ? "#262626" : "white"} 
