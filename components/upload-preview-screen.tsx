  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 w-full p-4 flex justify-center items-center relative">
        ...
        {/* 텍스트 디스플레이 영역 (클릭 시 모달 호출) */}
        <div className="absolute top-10 left-6 right-6 z-30 cursor-pointer" onClick={() => setIsTextModalOpen(true)}>
          {questionText ? (
            <p className="text-white text-2xl font-bold">{questionText}</p>
          ) : (
            <p className="text-white/50 text-2xl font-bold italic">질문을 입력하세요...</p>
          )}
        </div>

        {/* 툴바 */}
        <div className="absolute right-4 top-1/2 flex flex-col gap-4 z-40">
           <button onClick={() => setIsMosaicMode(!isMosaicMode)} className="p-3 bg-white/20 rounded-full"><Grip className="text-white" /></button>
           <button onClick={() => setIsEraserMode(!isEraserMode)} className="p-3 bg-white/20 rounded-full"><Eraser className="text-white" /></button>
           <button onClick={() => setIsTextModalOpen(true)} className="p-3 bg-white/20 rounded-full"><Type className="text-white" /></button>
           <button onClick={() => setIsPollModalOpen(true)} className="p-3 bg-white/20 rounded-full"><Plus className="text-white" /></button>
        </div>
      </div>
