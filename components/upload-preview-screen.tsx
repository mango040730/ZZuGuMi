...
export function UploadPreviewScreen({ 
  ...
  const handleUploadClick = async () => {
    ...
    if (!capturedImage) { onUpload(questionText, capturedImage, pollOptions); return }

    setIsUploading(true)
    try {
      const container = containerRef.current
      if (!container) { onUpload(questionText, capturedImage, pollOptions); return }
      ...
      if (questionText.trim().length > 0) {
        ...
        ctx.shadowColor = "rgba(0, 0, 0, 0.6)"; ctx.shadowBlur = 12 * textScale; ctx.shadowOffsetY = 4 * textScale
        const padding = 24 * textScale
        questionText.split("\n").forEach((line, i) => {
          ctx.fillText(line, padding, padding + i * fontSize * 1.3, outWidth - padding * 2)
        })
      }
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col overflow-hidden">
      <div className="p-4 flex justify-end items-center z-50 shrink-0">
        <button onClick={onClose} className="w-10 h-10 flex items-center justify-center"><X className="w-7 h-7 text-white" strokeWidth={1.5} /></button>
      </div>
        ...
        <div 
          ...
          onPointerDown={(e) => {
            if (step === "mosaic") {
              ...
              if (mode === "mosaic") setCurrentStroke([point])
              else setStrokes(prev => prev.filter(stroke => !stroke.some(p => Math.hypot(p.x - point.x, p.y - point.y) < 30)))
            }
          }}
          onPointerMove={(e) => {
            if (step === "mosaic" && isPointerDown) {
              const rect = e.currentTarget.getBoundingClientRect()
              const point = { x: e.clientX - rect.left, y: e.clientY - rect.top }
              if (mode === "mosaic") setCurrentStroke(prev => [...prev, point])
              else setStrokes(prev => prev.filter(stroke => !stroke.some(p => Math.hypot(p.x - point.x, p.y - point.y) < 30)))
            }
          }}
          onPointerUp={() => {
        ...
        >
          ...
          {step === "mosaic" && (
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-40">
              <button onClick={() => setMode("mosaic")} className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${mode === "mosaic" ? "bg-white" : "bg-black/40 backdrop-blur-md"}`}>
