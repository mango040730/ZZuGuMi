        ...
        <div ref={containerRef} className={`relative h-full max-h-[75vh] aspect-[22/29] bg-zinc-900 rounded-3xl overflow-hidden shrink-0 ${step === "mosaic" && (isMosaicMode || isEraserMode) ? "touch-none cursor-crosshair" : ""}`}
          ...
          {(step === "text" || questionText.length > 0) && (
            <div className={`absolute top-0 left-0 right-0 p-6 flex flex-col z-30 ${step !== "text" ? 'pointer-events-none' : ''}`}>
              {step === "text" ? (
                <textarea ref={textareaRef} value={questionText} onChange={(e) => setQuestionText(e.target.value)} placeholder="궁금한 점을 적어보세요" className="w-full bg-transparent text-white text-2xl sm:text-3xl font-bold resize-none outline-none drop-shadow-xl" rows={1} />
              ) : (
          ...
          {step === "mosaic" && (
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-40" onPointerDown={(e) => e.stopPropagation()}>
              ...
              <button onClick={handleEditText} className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg bg-black/40 backdrop-blur-md text-white border border-white/20"><Type className="w-5 h-5" /></button>
