...
export function UploadPreviewScreen({ 
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 w-full px-4 pb-2 min-h-0 flex justify-center items-center">
        <div ref={containerRef} className={`relative h-full max-h-[75vh] aspect-[22/29] bg-zinc-900 rounded-3xl overflow-hidden shrink-0 ${step === "mosaic" && (isMosaicMode || isEraserMode) ? "touch-none cursor-crosshair" : ""}`}
          ...
          {(step === "text" || questionText.length > 0) && (
            <div className={`absolute top-0 left-0 right-0 p-6 flex flex-col z-30`} onClick={() => setIsTextModalOpen(true)}>
              {questionText.length > 0 ? (
                <div className="w-full text-white text-2xl sm:text-3xl font-bold drop-shadow-xl whitespace-pre-wrap">{questionText}</div>
              ) : (
                <div className="w-full text-white/70 text-2xl font-bold italic">궁금한 점을 적어보세요</div>
              )}
            </div>
          )}
          ...
          {step === "mosaic" && (
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 z-40" onPointerDown={(e) => e.stopPropagation()}>
              ...
              <button onClick={() => setIsTextModalOpen(true)} className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg bg-black/40 backdrop-blur-md text-white border border-white/20"><Type className="w-5 h-5" /></button>
              <button onClick={() => setIsPollModalOpen(true)} className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg bg-black/40 backdrop-blur-md text-white border border-white/20"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2.5 12H5.88197C6.56717 12 7.19357 12.3871 7.5 13C7.80643 13.6129 8.43283 14 9.11803 14H14.882C15.5672 14 16.1936 13.6129 16.5 13C16.8064 12.3871 17.4328 12 18.118 12H21.5M8.96656 4H15.0334C16.1103 4 16.6487 4 17.1241 4.16396C17.5445 4.30896 17.9274 4.5456 18.2451 4.85675C18.6043 5.2086 18.8451 5.6902 19.3267 6.65337L21.4932 10.9865C21.6822 11.3645 21.7767 11.5535 21.8434 11.7515C21.9026 11.9275 21.9453 12.1085 21.971 12.2923C22 12.4992 22 12.7105 22 13.1331V15.2C22 16.8802 22 17.7202 21.673 18.362C21.3854 18.9265 20.9265 19.3854 20.362 19.673C19.7202 20 18.8802 20 17.2 20H6.8C5.11984 20 4.27976 20 3.63803 19.673C3.07354 19.3854 2.6146 18.9265 2.32698 18.362C2 17.7202 2 16.8802 2 15.2V13.1331C2 12.7105 2 12.4992 2.02897 12.2923C2.05471 12.1085 2.09744 11.9275 2.15662 11.7515C2.22326 11.5535 2.31776 11.3645 2.50675 10.9865L4.67331 6.65337C5.1549 5.69019 5.3957 5.2086 5.75495 4.85675C6.07263 4.5456 6.45551 4.30896 6.87589 4.16396C7.35125 4 7.88969 4 8.96656 4Z" /></svg></button>
            </div>
          )}
        </div>
      </div>
      ...
      {isTextModalOpen && <TextModal onClose={() => setIsTextModalOpen(false)} onRegister={(text) => { setQuestionText(text); setIsTextModalOpen(false); if(step === "text") setStep("text"); }} initialValue={questionText} />}

      <div className="flex justify-center pt-4 pb-8 px-4 z-50 shrink-0">
        {step === "text" ? (
          <button onClick={handleNextStep} disabled={questionText.trim().length === 0} className="w-full max-w-[calc(100vw-32px)] py-4 bg-[#FF6200] rounded-full text-white text-base font-bold disabled:opacity-50 shadow-lg">다음</button>
        ) : (
